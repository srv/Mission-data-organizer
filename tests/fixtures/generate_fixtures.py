...
The output layout is::
    ...
    <output-dir>/flat/bags/   # 2 missions + 1 basic + 1 .bag.active + operator .txt notes
    <output-dir>/flat/logs/   # mk_ii, multibeam, mission_reports, iquaview, emus_bms, blackfly_s, flir_spinnaker_stereo_camera, cola2_log
...
def _write_report(path: Path, start: str, end: str) -> None:
    """Write a mission_report.md with a real Start/End header block.

    ``start`` / ``end`` are ``YYYY/MM/DD HH:MM:SS`` UTC strings (the format
    iquaview writes and ``report_inspector`` parses).
    """
    ...
    path.write_text(
        ...
        f"- Mission completed on {start.split()[0]} at {start.split()[1]}\n"
        f"- Start time: {start}\n"
        f"- End time:   {end}\n"
    )
...
def main(out_dir: Path) -> int:
    ...
    # mission_reports — real headers spanning each mission's bag window. Each
    # report's Start time is a few seconds BEFORE its bag's internal start
    # (iquaview writes the report at mission-planning time), so content-aware
    # overlap matching is exercised — filename TS alone would demote it (#8).
    _write_report(
        flat_logs / "mission_reports" / "2026-05-04_09-03-30_mission_report.md",
        "2026/05/04 09:03:25", "2026/05/04 09:05:00",
    )
    _write_report(
        flat_logs / "mission_reports" / "2026-05-04_10-50-33_mission_report.md",
        "2026/05/04 10:50:28", "2026/05/04 10:55:00",
    )

    # Camera image folders → mission raw/. blackfly_s for mission 1, FLIR stereo
    # for mission 2; both are whole folders of paired stereo images.
    ...
    flir = flat_logs / "flir_spinnaker_stereo_camera" / "2026-05-04-10-50-33"
    flir.mkdir(parents=True, exist_ok=True)
    (flir / "1700000000000000000_port.jpg").touch()
    (flir / "1700000000000000000_starboard.jpg").touch()

    # Root-level operator .txt notes → day folder by mtime. Names are irregular
    # (non-padded date / dateless readme), so mtime, not the filename, decides
    # the day. Set mtime to midday on the mission day (UTC).
    txt_mtime = _utc_unix(2026, 5, 4, 12, 0, 0)
    for txt_name in ("2026-05-04_ondeck.txt", "2026-05-04_onwater.txt",
                     "README.txt"):
        p = flat_bags / txt_name
        p.write_text("operator note\n")
        os.utime(p, (txt_mtime, txt_mtime))

    # Out-of-scope source — present but the walker must ignore it.
